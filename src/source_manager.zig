...
pub fn download(ctx: *mere.Context, config: DownloadConfig) !DownloadResult {
        ...
        var prepared = std.ArrayList(PreparedSource){};
        defer {
        ...
        var batch_requests = std.ArrayList(download_mod.BatchDownloadRequest){};
        ...
        while (i < config.sources.len) : (i += 1) {
            ...
            if (expected_hash != null) {
                const file_exists = blk: {
                    std.fs.accessAbsolute(cache_path_owned, .{}) catch break :blk false;
                    break :blk true;
                };
                if (file_exists) {
                    ...
                    if (std.mem.eql(u8, local_hash, expected_hash.?)) {
                        const file_size = blk: {
                            const file = std.fs.openFileAbsolute(cache_path_owned, .{}) catch break :blk 0;
                            defer file.close();
        ...
        for (prepared.items) |entry| {
            const cache_path_z = try ctx.allocator.dupeZ(u8, entry.cache_path);
            defer ctx.allocator.free(cache_path_z);
            const workspace_path_z = try ctx.allocator.dupeZ(u8, entry.workspace_source_path);
            defer ctx.allocator.free(workspace_path_z);
            std.posix.linkat(std.posix.AT.FDCWD, cache_path_z, std.posix.AT.FDCWD, workspace_path_z, 0) catch |link_err| {
                ctx.debug("hardlink failed ({s}), copying instead: {s} -> {s}", .{ @errorName(link_err), entry.cache_path, entry.workspace_source_path });

                var cache_file = std.fs.openFileAbsolute(entry.cache_path, .{}) catch |err| {
                    ctx.setDiagnosticContext(entry.cache_path, "failed to open cached source");
                    ctx.allocator.free(cache_dir);
                    return mapFsError(err);
                };
                defer cache_file.close();

                var workspace_file = std.fs.createFileAbsolute(entry.workspace_source_path, .{}) catch |err| {
                ...
                while (true) {
                    const n = cache_file.read(&buf) catch |err| {
                        ctx.setDiagnosticContext(entry.cache_path, "failed to read cached source");
                        ctx.allocator.free(cache_dir);
                        return mapFsError(err);
                    };
                    if (n == 0) break;
                    workspace_file.writeAll(buf[0..n]) catch |err| {
                        ctx.setDiagnosticContext(entry.workspace_source_path, "failed to write workspace source");
                        ctx.allocator.free(cache_dir);
                        return mapFsError(err);
                    };
                }
...
test "download handles pre-existing files and hardlinks correctly" {
    ...
    var cache_file = try std.fs.createFileAbsolute(cached_file_path, .{});
    defer cache_file.close();
    try cache_file.writeAll(src_content);
    ...
    const workspace_content = try std.fs.cwd().readFileAlloc(test_env.ctx.allocator, workspace_file, 1024);
...
test "download writes cache file with expected content (atomic rename smoke)" {
    ...
    const cached_content = try std.fs.cwd().readFileAlloc(test_env.ctx.allocator, cached_file, 1024);
...
test "download reports read errors in cache-to-workspace copy fallback" {
    ...
    try std.fs.cwd().makePath(cache_entry_dir);
...
test "download reports permission errors in cache-to-workspace fallback" {
    ...
    {
        var cache_file = try std.fs.createFileAbsolute(cache_file_path, .{});
        defer cache_file.close();
        try cache_file.writeAll(src_content);
    }
    ...
    var sources_dir_handle = try std.fs.openDirAbsolute(sources_dir, .{});
    defer sources_dir_handle.close();
    try sources_dir_handle.chmod(0o555);
