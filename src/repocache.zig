...
/// - CorruptData: Rollback state file is malformed
const Std = errors.StandardErrors;
pub const RepoCacheError = Std.OutOfMemory || Std.FileSystem || Std.Network || Std.PermissionDenied || Std.SignatureInvalid || Std.CorruptData || error{RollbackDetected};
...
pub const RepoCache = struct {
    ...
    pub fn sync(
        ...
        if (sync_result) |_| {
            ...
            // Reconcile rollback state: prune entries for packages that no
            // longer exist in the repo.  This prevents stale rollback state
            // (e.g. from a repo-remove) from blocking future installs.
            self.reconcileRollbackState() catch |err| {
                self.ctx.debug("failed to reconcile rollback state for repo {s}: {s}", .{ self.name, @errorName(err) });
            };
            return;
        } else |err| {
    ...
    /// Check rollback protection for a package's created_at timestamp.
    pub fn checkRollbackState(
        self: *RepoCache,
        signer_fingerprint: []const u8,
        name: []const u8,
        arch: []const u8,
        created_at: u64,
    ) RepoCacheError!void {
        var state = try loadRollbackState(self, self.ctx.allocator);
        defer freeRollbackState(&state, self.ctx.allocator);

        const key = try makeRollbackKey(self.ctx.allocator, signer_fingerprint, name, arch);
        defer self.ctx.allocator.free(key);

        if (state.get(key)) |stored| {
            if (created_at < stored) {
                return RepoCacheError.RollbackDetected;
            }
        }
    }

    /// Clear rollback protection state for a specific (name, arch) pair.
    /// Used after repo-remove so the package can be re-installed without
    /// triggering false rollback detection.
    pub fn clearRollbackState(
        self: *RepoCache,
        name: []const u8,
        arch: []const u8,
    ) RepoCacheError!void {
        ...
        while (iter.next()) |entry| {
            // Key format is "signer\tname\tarch"
            var key_iter = std.mem.splitScalar(u8, entry.key_ptr.*, '\t');
            _ = key_iter.next(); // skip signer
            const entry_name = key_iter.next() orelse continue;
            const entry_arch = key_iter.next() orelse continue;

            if (std.mem.eql(u8, entry_name, name) and std.mem.eql(u8, entry_arch, arch)) {
                to_remove.append(self.ctx.allocator, entry.key_ptr.*) catch {
                    return RepoCacheError.OutOfMemory;
                };
            }
        }
        ...
        for (to_remove.items) |key| {
            if (state.fetchRemove(key)) |kv| {
                self.ctx.allocator.free(kv.key);
            }
        }

        try writeRollbackState(self, &state);
    }

    /// Reconcile rollback state against the current repo contents.
    /// Removes entries for (name, arch) pairs that no longer exist in the repo.
    /// All entries are treated as belonging to the current trust context,
    /// since cache directories are now scoped by trust context.
    fn reconcileRollbackState(self: *RepoCache) RepoCacheError!void {
        // Initialize the repo DB so we can query it
        try self.ensureRepository();
        const repo = self.repository orelse return;

        const allocator = self.ctx.allocator;

        // Get all (name, arch) pairs currently in the repo
        var live_packages = repo.db.getDistinctPackageNameArch(allocator) catch {
            return RepoCacheError.FileSystem;
        };
        defer {
            for (live_packages.items) |item| {
                allocator.free(item.name);
                allocator.free(item.arch);
            }
            live_packages.deinit(allocator);
        }

        // Build a lookup set of "name\tarch" for fast membership checks
        var live_set = std.StringHashMap(void).init(allocator);
        defer {
            var key_iter = live_set.keyIterator();
            while (key_iter.next()) |k| allocator.free(k.*);
            live_set.deinit();
        }
        for (live_packages.items) |item| {
            const lookup_key = std.fmt.allocPrint(allocator, "{s}\t{s}", .{ item.name, item.arch }) catch {
                return RepoCacheError.OutOfMemory;
            };
            live_set.put(lookup_key, {}) catch {
                allocator.free(lookup_key);
                return RepoCacheError.OutOfMemory;
            };
        }

        // Load current rollback state and prune stale entries
        ...
        while (iter.next()) |entry| {
            // Key format is "signer\tname\tarch"
            var key_parts = std.mem.splitScalar(u8, entry.key_ptr.*, '\t');
            _ = key_parts.next() orelse continue; // signer (unused, all entries belong to current trust context)
            const entry_name = key_parts.next() orelse continue;
            const entry_arch = key_parts.next() orelse continue;

            const lookup_key = std.fmt.allocPrint(allocator, "{s}\t{s}", .{ entry_name, entry_arch }) catch {
                return RepoCacheError.OutOfMemory;
            };
            defer allocator.free(lookup_key);

            if (!live_set.contains(lookup_key)) {
        ...
        for (to_remove.items) |key| {
            if (state.fetchRemove(key)) |kv| {
                self.ctx.debug("pruned stale rollback entry: {s}", .{kv.key});
    ...
    /// Update rollback protection state after accepting a package.
    pub fn updateRollbackState(
        self: *RepoCache,
        signer_fingerprint: []const u8,
        name: []const u8,
        arch: []const u8,
        ...
        if (state.getPtr(key)) |ptr| {
            if (created_at > ptr.*) {
                ptr.* = created_at;
            }
        } else {
            try state.put(try self.ctx.allocator.dupe(u8, key), created_at);
        }
...
const rollback_state_filename = "rollback_state.kdl";
const last_sync_filename = "last_sync.kdl";

fn rollbackStatePath(self: *RepoCache) RepoCacheError![]const u8 {
    return std.fs.path.join(self.ctx.allocator, &.{ self.cache_dir, rollback_state_filename }) catch {
        return RepoCacheError.OutOfMemory;
    };
}
...
fn makeRollbackKey(
    ...
    name: []const u8,
    arch: []const u8,
) RepoCacheError![]const u8 {
    return std.fmt.allocPrint(allocator, "{s}\t{s}\t{s}", .{ signer_fingerprint, name, arch }) catch {
        return RepoCacheError.OutOfMemory;
    };
}

fn loadRollbackState(
    self: *RepoCache,
    allocator: std.mem.Allocator,
) RepoCacheError!std.StringHashMap(u64) {
    var state = std.StringHashMap(u64).init(allocator);
    ...
    if (stat.size > 1024 * 1024) {
        return RepoCacheError.CorruptData;
    }
    ...
    for (nodes.items) |node| {
        if (!std.mem.eql(u8, node.name, "rollback")) continue;
        const signer = node.getStringProperty("signer") orelse return RepoCacheError.CorruptData;
        const name = node.getStringProperty("name") orelse return RepoCacheError.CorruptData;
        const arch = node.getStringProperty("arch") orelse return RepoCacheError.CorruptData;
        const created_at_i = node.getIntProperty("created_at") orelse return RepoCacheError.CorruptData;
        if (created_at_i < 0) return RepoCacheError.CorruptData;
        const created_at: u64 = @intCast(created_at_i);

        const key = try makeRollbackKey(allocator, signer, name, arch);
        if (state.getPtr(key)) |ptr| {
            if (created_at > ptr.*) {
                ptr.* = created_at;
            }
            ...
            state.put(key, created_at) catch {
                allocator.free(key);
                return RepoCacheError.OutOfMemory;
            };
        }
...
fn freeRollbackState(state: *std.StringHashMap(u64), allocator: std.mem.Allocator) void {
...
fn writeRollbackState(self: *RepoCache, state: *const std.StringHashMap(u64)) RepoCacheError!void {
    const path_buf = try rollbackStatePath(self);
    ...
    while (iter.next()) |entry| {
        var key_iter = std.mem.splitScalar(u8, entry.key_ptr.*, '\t');
        const signer = key_iter.next() orelse return RepoCacheError.CorruptData;
        const name = key_iter.next() orelse return RepoCacheError.CorruptData;
        const arch = key_iter.next() orelse return RepoCacheError.CorruptData;

        const line = std.fmt.allocPrint(self.ctx.allocator, "rollback signer=\"{s}\" name=\"{s}\" arch=\"{s}\" created_at={d}\n", .{
            signer,
            name,
            arch,
            entry.value_ptr.*,
